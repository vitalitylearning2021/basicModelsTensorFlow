...
La prima è ricorrere ad una approssimazione delle derivate con le differenze finite, così come abbiamo fatto nel capitolo precedente. Tuttavia, l'operazione di differenziazione numerica tramite le differenze finite deve essere effettuata con cura per evitare che gli errori numerici possano diventare rapidamente significativi.\\
La seconda è utilizzare un tool di derivazione simbolica, come quelli disponibili in Matlab o in WolframAlpha. Tuttavia, anche la derivazione simbolica deve essere usata con cautela in quanto la dimensione del risultato può essere molto più grande rispetto all'espressione di partenza.\\
Una terza possibilità è quella di riguardare la formula da differenziare come una serie di operazioni elementari implementabili da un qualsiasi tradizionale linguaggio programmazione. Applicando le regole delle derivate alle singole operazioni elementari si vuole ottenere un codice che permette di calcolare numericamente le derivate. Per chiarire meglio l'idea, supponiamo che l'espressione di $f(x_1,x_2)$ sia la seguente:
